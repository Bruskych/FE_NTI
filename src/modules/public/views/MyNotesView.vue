...
<template>
    <div class="mx-auto max-w-7xl px-4 py-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-900">Moje poznámky</h1>
            <p class="text-sm text-slate-600">
                Tu vidíš aj svoje drafty
            </p>
        </div>

        <p v-if="notesStore.loadingMine" class="text-slate-600">Načítavam...</p>
        <p v-else-if="notesStore.error" class="text-red-600">{{ notesStore.error }}</p>

        <div v-else class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            <RouterLink v-for="note in notesStore.myNotes" :key="note.id" :to="`/notes/${note.id}`"
        ...
        <AppPagination v-if="notesStore.paginationMine" :pagination="notesStore.paginationMine" @change="loadPage" />
    </div>
</template>