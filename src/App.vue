...
/**
 * Интеллектуальный скролл к секциям главной страницы.
 * Если пользователь на другой странице — сначала редиректит на главную,
 * ждет обновления DOM и плавно прокручивает к нужному блоку.
 */
const navigateToSection = async (sectionId: string) => {
  if (route.path !== '/') {
    await router.push('/')
    await nextTick() // Ожидаем, пока Vue полностью отрендерит главную страницу
  }
      ...
      <div class="footer-grid-system">
        ...
        <div class="footer-block block-center">
          ...
          <nav class="block-links">
            ...
            <router-link to="/about-us" class="interactive-link-btn router-style">
              {{ $t('footer.about_us') || 'O nás' }}
            </router-link>
          </nav>
        </div>
...
/* Стили ссылок и кнопок с эффектом подчеркивания */
